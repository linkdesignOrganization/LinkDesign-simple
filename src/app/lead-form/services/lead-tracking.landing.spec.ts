 ...
 * sistema, las industrias, el hub de software de Costa Rica con sus fichas y,
...
const ESPERADO: Record<string, Expectativa> = {
  ...
  'desarrollo-de-software-costa-rica/:slug': 'software'
};
...
const SLUG_DE_EJEMPLO: Record<string, string> = {
  ...
  'desarrollo-de-software-costa-rica/:slug': SOFTWARE_CR_CASE_SLUGS[0]
};
...
function crearClasificador(): (pathname: string) => SourceLanding {
  const location = { pathname: '/', href: 'https://linkdesign.cr/', search: '', hostname: 'linkdesign.cr' };
  const documento = {
    location,
    referrer: '',
