...
/** Duración de cada video de demo en ISO 8601 (medida con ffprobe, auditoría SEO 2026-09-08). */
const CASE_VIDEO_DURATIONS: Record<SoftwareCrCaseSlug, string> = {
  pulso: 'PT42S',
  cumbre: 'PT38S',
...
/**
 * Ficha de un sistema de demostración: /desarrollo-de-software-costa-rica/:slug, en ES y EN
 ...
 * Contenido y rótulos por idioma en software-cr-cases-content.ts (getters sobre `lang()`); los
 ...
 * `VideoObject` en el <head>, como los demos de /software. Misma condición que el hub: noindex
 * (meta + cabecera, ver seo-content.ts) mientras esté en revisión.
 */
@Component({
  selector: 'app-software-cr-case-page',
  standalone: true,
        ...
        <div class="cc-dark" appDarkZone>
          <section class="cc-section cc-cost" appTrackSection="caso-costo">
            ...
            <div class="cc-cost__actions">
              ...
              <a class="cc-cost__link" [routerLink]="'/desarrollo-de-software-costa-rica' | localizeUrl" fragment="precios">
                {{ l().costLink }} →
              </a>
            </div>
          ...
          <section class="cc-section cc-others" appTrackSection="caso-otros">
            ...
            <ul class="cc-others__list">
              @for (o of others(); track o.slug) {
                <li>
                  <a class="cc-other" [routerLink]="('/desarrollo-de-software-costa-rica/' + o.slug) | localizeUrl">
            ...
            <a class="cc-back" [routerLink]="'/desarrollo-de-software-costa-rica' | localizeUrl">← {{ l().backToHub }}</a>
          </section>
        </div>
      </article>
...
export class SoftwareCrCasePageComponent implements OnDestroy {
  ...
  protected readonly l = computed(() => getSoftwareCrCaseLabels(this.lang()));
  ...
  protected readonly d = computed(() => getSoftwareCrCase(this.slug(), this.lang()));

  // Los otros demos (en el idioma activo), para navegar entre fichas.
  protected readonly others = computed(() =>
    getSoftwareCrCases(this.lang()).filter((c) => c.slug !== this.slug())
  );
  ...
  protected readonly context = computed<SystemContext | null>(() => {
    const s = getSoftwareCrCase(this.slug(), 'es');
    return s ? { name: `Demo ${s.name} (${s.category})`, slug: `desarrollo-de-software-costa-rica/${s.slug}` } : null;
  });
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const data = {
        ...
        inLanguage: this.lang() === 'en' ? 'en-US' : 'es-CR',
