...
@Component({
  ...
  template: `
    @if (d(); as s) {
      <article class="id">
        ...
        <header class="id-hero">
          ...
          <div class="id-hero__art" aria-hidden="true">
            <span class="id-hero__icon">
              @switch (s.icon) {
                @case ('factory') {
                  <svg lucideFactory [size]="168" [strokeWidth]="0.6"></svg>
                }
                @case ('truck') {
                  <svg lucideTruck [size]="168" [strokeWidth]="0.6"></svg>
                }
                @case ('health') {
                  <svg lucideHeartPulse [size]="168" [strokeWidth]="0.6"></svg>
                }
                @case ('briefcase') {
                  <svg lucideBriefcase [size]="168" [strokeWidth]="0.6"></svg>
                }
                @case ('wrench') {
                  <svg lucideWrench [size]="168" [strokeWidth]="0.6"></svg>
                }
                @case ('dumbbell') {
                  <svg lucideDumbbell [size]="168" [strokeWidth]="0.6"></svg>
                }
                @case ('education') {
                  <svg lucideGraduationCap [size]="168" [strokeWidth]="0.6"></svg>
                }
              }
            </span>
          </div>
        ...
        <!-- 04 — Cómo trabajamos juntos (roles: tú / nosotros) -->
  ...
  styles: `
    ...
    /* Icono grande, a la derecha y centrado vertical: ancla visual del bloque (ink en "tú",
...
export class IndustryDetailPageComponent implements AfterViewInit, OnDestroy {
  ...
  protected readonly buildCards = computed<BuildCard[]>(() => {
    ...
    return detail.couldBuild.items.map((it) => ({
      kind: it.kind,
      kicker: labels[it.kind],
      text: it.text,
    }));
  });
