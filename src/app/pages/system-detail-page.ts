...
import { getSoftwareCrCaseForSystem } from './software-cr-cases-content';
import { getSoftwareCrPriceForSystem } from './software-cr-content';
...
@Component({
    ...
    @if (d(); as s) {
      <article class="sd">
            ...
            <div class="sd-demo__grid">
              <div class="sd-demo__text">
                ...
                <div class="sd-demo__actions">
                  ...
                  <a class="sd-demo__link" [routerLink]="('/desarrollo-de-software-costa-rica/' + ex.slug) | localizeUrl">{{ t().demoFicha }} →</a>
                </div>
              </div>
              <a
        ...
        <div class="sd-dark" appDarkZone>
          ...
          <!-- Cuánto cuesta en Costa Rica: el rango del hub para este tipo de sistema (si tiene fila
               propia) y el enlace al hub /desarrollo-de-software-costa-rica (a la página, no al ancla de precios).
          ...
          <section class="sd-section sd-cost sd-reveal" appTrackSection="sistema-costo">
            ...
            <a class="sd-cost__link" [routerLink]="'/desarrollo-de-software-costa-rica' | localizeUrl">{{ t().costLink }} →</a>
          </section>
  ...
  styles: `
    ...
    /* ── Cuánto cuesta en Costa Rica (zona oscura): dos datos, texto y enlace al hub ── */
    .sd-cost__stats {
      display: grid;
...
export class SystemDetailPageComponent implements AfterViewInit, OnDestroy {
  ...
  protected readonly demo = computed(() => getSoftwareCrCaseForSystem(this.slug(), this.lang()));
  ...
  protected readonly priceView = computed(() => {
    const row = getSoftwareCrPriceForSystem(this.slug(), this.lang());
  ...
  protected readonly demoIntro = computed(() => {
    ...
    if (!ex) return '';
    if (this.lang() === 'en') {
      return `We built ${ex.name} as a demo so you can browse a system of this kind with sample data.`;
    }
    const category = ex.category.replace(/^(Sistema|Software)/, (m) => m.toLowerCase());
    return `Construimos ${ex.name}, un ${category}, como demostración para que navegues un sistema de este tipo con datos de ejemplo.`;
  });
  ...
  // Footer del sitio (mismos datos que contact-page de Link Design).
  ...
  // la hidratación o el HMR del dev server reemplazan: las secciones quedaban en opacity 0 y se
  // veían como espacios vacíos enormes (mismo arreglo que en la landing de software CR).
  private revealRaf = 0;
  ...
  constructor() {
    // Slug inexistente → 404 con marca (LinkDesign tiene su propia página /404).
    effect(() => {
  ...
  // CTA "Escríbenos": scroll suave al formulario del footer (#hablemos), sin navegar.
...
// Rótulos de UI (nombres de las 9 secciones del contenido + textos de interfaz). No es copy
// nuevo: son los encabezados de sección del documento aprobado, usados como títulos.
const SECTION_LABELS = {
  es: {
    ...
    demoNote: 'Es un ejemplo, no el sistema de un cliente ni lo que vas a recibir. Cada desarrollo se diseña sobre tu operación.',
    demoCta: 'Navegar el demo',
    demoFicha: 'Ver la ficha',
    costTitle: 'Cuánto cuesta en Costa Rica',
    costRange: 'Inversión',
    costTimeline: 'Plazo',
    costText:
    ...
    costTextGeneral:
      'Un sistema a la medida con nosotros cuesta entre USD 1.500 y 15.000, según el alcance. El precio queda cerrado antes de empezar. Anticipo y saldo contra entrega, pagos por hito o cuota mensual con soporte incluido.',
    costNote:
  ...
  en: {
    ...
    demoNote: "It is an example, not a client's system or what you will receive. Every build is designed around your operation.",
    demoCta: 'Browse the demo',
    demoFicha: 'See the case',
    costTitle: 'What it costs in Costa Rica',
    ...
    costTextGeneral:
      'A custom system with us costs between USD 1,500 and 15,000, depending on scope. The price is fixed before we start. Deposit and balance on delivery, milestone payments or a monthly fee with support included.',
    costNote:
