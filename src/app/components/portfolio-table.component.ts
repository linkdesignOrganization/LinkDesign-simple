...
export type PortfolioRow = {
   ...
   * path → slide de video · '' → slide de solo póster (proyecto sin video, ej. Asembis) ·
   * null → SIN render de hero: excluido del carrusel (regla anti-desconexión, 2ª capa).
   */
  heroSrc?: string | null;
};
...
export class PortfolioTableComponent implements OnDestroy {
  ...
  // Precarga por proximidad (ver setupNearbyPreload): reemplaza al prefetch de los 25 clips.
  ...
  private setupViewer(): void {
    ...
    if (inline) {
      ...
      if (!reduced) {
        ...
        // Cada clip se descarga cuando su fila se acerca, no los 25 de entrada.
  ...
  //
  // Antes se hacía un `<link rel="prefetch">` de los 25 clips apenas cargaban los logos: 18,5 MB que
  // se descargaban siempre, aunque el visitante nunca bajara hasta el portafolio, y con los 25 pedidos
  // compitiendo entre sí por las ~6 conexiones del navegador — así que el clip que hacía falta en dos
  // segundos podía estar vigésimo en la cola. Justo en conexión lenta, que es cuando importaba.
  //
